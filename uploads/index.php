...
$navList = buildNavigationList('', $classifications);
...
switch ($action) {
    case 'upload':
        ...
        if($imageCheck){
            $message = '<p class="alert-message alert-danger">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p class="alert-message alert-danger">You must select a vehicle and image file for the vehicle.</p>';
        } else {
            ...
            if ($result) {
                $message = '<p class="alert-message alert-success">The upload succeeded.</p>';
            } else {
                $message = '<p class="alert-message alert-danger">Sorry, the upload failed.</p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p class='alert-message alert-success'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p class='alert-message alert-danger'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p class="alert-message alert-danger">Sorry, no images could be found.</p>';
        }
